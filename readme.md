...
a running summary (generated by the background model - see below)
instead of being sent in full every time. The last 10 messages always
go through verbatim. Nothing in your saved chat history is ever
shortened — only what gets sent to the model behind the scenes.

### Background model

Document summaries and the conversation compression above both run on
one small "background" model - by default whichever `.gguf` is smallest
(normally Qwen3.5 2B), or a specific one you pin via `utility_model` in
`config.json`. It's chosen independently of the active/inactive toggle
in Settings → Modeller, so switching it off there hides it from the
main chat picker (handy if you don't want to select it by accident -
small models tend to ramble incoherently on open-ended questions) without
affecting its background job.

It also isn't kept resident: it loads only for the brief moment a
summary is being generated and is unloaded immediately after (see
`model_manager.unload_utility_model`), rather than sitting in RAM the
whole time you're chatting with a different model. Reloading it costs a
few seconds the next time it's needed - a good trade for freeing the
~1-2 GB it would otherwise hold onto continuously.
...
- An active/inactive toggle — inactive models don't appear in the main
  chat picker, so you can't select them by accident. This is independent
  of the background/utility model choice: turning off the small model
  used for summaries (Qwen3.5 2B by default) hides it from chat but it
  still keeps doing its background job - see "Background model" below
